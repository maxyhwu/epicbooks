...
app.put('/api/genRandomBooks', (req:any, res:any) => {
    const booksCollection = db.collection('books') as mongoose.Collection;
    const randomBook = {
        title: "",
        author: "",
    ...
    if(!booksCollection.findOne({title: randomBook.title})) {
        booksCollection.insertOne(randomBook).then(() => {
            res.send(randomBook);
        });
    }
    else {
        booksCollection.updateOne({title: randomBook.title}, {$set: randomBook}).then(() => {
            res.send(randomBook);
        }).catch((err) => {
            console.error(err);
        });
