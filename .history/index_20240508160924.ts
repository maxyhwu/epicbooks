...
app.put('/api/updateRandomGenBooks', (req:any, res:any) => {
    const booksCollection = db.collection('books') as mongoose.Collection;
    const randomBook = {
        title: "",
        author: "",
    ...
    if(!booksCollection.findOne({title: randomBook.title})) {
        booksCollection.insertOne(randomBook);
    }
    else {
        booksCollection.updateOne({title: randomBook.title}, {$set: randomBook});
    
    }
    db.collection('books').updateOne({title: randomBook.title}, {$set: randomBook}, {upsert: true});
});
