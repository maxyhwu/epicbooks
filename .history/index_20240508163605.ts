...
app.put('/api/genRandomBooks', async(req:any, res:any) => {
    const randomBook = {
        ...
        genre: Array(Math.floor(Math.random() * 5)).fill(Math.random().toString(36).substring(7)),
    ...
    await booksModel.insertOne(randomBook).then(() => {
...
app.delete('/api/deleteRandomBooks', async(req:any, res:any) => {
    await db.collection('books').deleteMany({}).then(() => {
