...
app.post('/api/forgotPassword', async(req:any, res:any) => {
    const username = req.body === undefined ? req.body.username : 'nullUser' as string;
    const email = req.body === undefined ? req.body.email : 'nullUser' as string;
...
app.put('/api/genRandomBooks', async(req:any, res:any) => {
    ...
    for(let i = 0; i < numBooks; i++) {
        const randomBook = {
            ...
            title: uniqueNamesGenerator(randomTitleConfig) as string,
...
app.put('/api/genNullUser', async(req:any, res:any) => {
    ...
    await usersModel.findOne({username: 'nullUser'}).then(async(user) => {
        if(user) {
